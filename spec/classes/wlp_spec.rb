    ...
    on_supported_os.each do |os, facts|
      context "on #{os}" do
        ...
        context "wlp class with install_src set" do
          ...
          it { is_expected.to contain_archive('/opt/ibm/wlp.zip').with({ 'extract_path' => '/opt/ibm' }) }
        ...
        context "wlp class with overriden location and user parameter" do
          ...
          it { is_expected.to contain_class('wlp::params') }
          it { is_expected.to contain_class('java') }
          it { is_expected.to contain_archive('/usr/local/wlp_test/wlp.zip').with({ 'extract_path' => '/usr/local/wlp_test' }) }
